...
export function Link({ to, href, ...props }) {
  ...
  return <NextLink href={url} className={styles.link} {...props} />
}
      ...
      {links &&
        links.map((link, i) => (
          <li key={link.ContentID}>
            <Button href={link.Data.href} variant={getVariant(i)}>
              {link.Data.text}
            </Button>
          </li>
        ))}
...
export function Avatar({ alt, src }) {
  return (
    <img alt={alt} src={src} className={styles.avatar} />
  )
}

export function Logo({ alt, src, size = "small" }) {
  return (
    <img
      alt={alt}
      src={src}
...
export function Icon({ alt, src, size = "medium" }) {
  return (
    <img
      alt={alt}
      src={src}
